...
propsText.append("asciidoctor.attributes=sourceDir=${targetDir},source-highlighter=prettify@,imagesoutDir=${targetDir}/microsite/output/images@,imagesDir=${micrositeContextPath}images@,targetDir=${targetDir},docDir=${docDir},projectRootDir=${new File(docDir).canonicalPath}@\n")
...
// Merge with existing jbake.properties if present
if (jbakeProps.exists()) {
    def existing = jbakeProps.text
    jbakeProps.write(existing + "\n" + propsText.toString())
} else {
    jbakeProps.write(propsText.toString())
}
