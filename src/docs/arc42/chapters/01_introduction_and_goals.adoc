...
* **Cross-platform CLI**: Provide a single command-line interface (`dtcw`) that works on Linux, macOS (including Apple Silicon), and Windows
...
|Identical output on Linux, macOS (including Apple Silicon), and Windows across local, Docker, and SDKMAN environments. (QS-2, QS-10)
...
|When something fails, docToolchain tells the user what to do — not what went wrong. Every error message is a concrete action: which file to close, which config to set, which command to run. (QS-16)
