...
|*Partly planned — not yet enforced in code.* Credentials should be supplied through environment variables or external config rather than committed into `docToolchainConfig.groovy`. The intended controls — masking properties whose names contain `credential`, `token`, or `secret` in all output, and suppressing stack traces for user-recoverable errors (ADR-8) — are **not implemented yet**: no redaction code exists, and `DtcRestClient` currently rethrows raw exceptions that can carry a token-bearing URL or header. Tracked as **R-008** (and gated on the ADR-8 `DtcException` carrier, R-004). This matters mainly for the CI / public-build context below, not for the local-trust default.
|T-002
