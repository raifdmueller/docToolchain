...
----
+
When `diagramServer = 'docker'`, dtcw checks if Docker is available, starts `yuzutech/kroki` as a container on port 8000, and sets the URL to `http://localhost:8000/`. The container persists between runs — dtcw only starts it if not already running.
+
...
** Supports **QS-16 (Actionable Guidance)**: If diagram rendering fails, the message is: "Setze `diagramServer = 'https://kroki.io/'` in docToolchainConfig.groovy" — not "install PlantUML from https://... and add it to PATH".
