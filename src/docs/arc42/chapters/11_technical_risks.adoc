...
|R-004
|*Residual inconsistent error handling.* The `DtcException` carrier now exists (`scripts/lib/DtcException.groovy`, ADR-8) and `generateHTML`, `generatePDF`, and `publishToConfluence` are migrated, but the remaining task scripts still mix thrown exceptions and `System.exit`/`println` until the rollout completes (QS-4, QS-3, QS-17).
|Medium
|Low
|LOW
