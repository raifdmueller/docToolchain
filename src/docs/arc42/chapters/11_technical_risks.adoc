...
|R-008
|*No secret redaction in output.* No code masks `credential`/`token`/`secret`-shaped values, and `DtcRestClient` rethrows raw exceptions. Harmless under the local-trust model, but a credential can leak into a public CI / PR build log.
|Low
|Medium
|LOW
|Ch8 *Security* concept — redact secret-shaped keys before any `println config` / error output; gated on the ADR-8 `DtcException` carrier (R-004). Closes T-002.
|===
