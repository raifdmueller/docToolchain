...
* **Status**: Accepted (for v4) — **carrier implemented; rollout in progress**. The hierarchy and top-level handler below exist in `scripts/lib/DtcException.groovy` (`DtcError.report()` maps each type to a differentiated exit code; `DtcError.redact()` masks secret-shaped values), unit-tested in `DtcExceptionSpec`. `generateHTML` is the reference adoption: it throws `DtcConfigException`/`DtcException` with guidance and routes everything through a single top-level handler (exit codes 0/1/2). The remaining task scripts still use `System.exit` + `println` and are migrated incrementally (risk **R-004**). `redact()` also delivers the T-002 secret-redaction mitigation for script-level error output (Chapter 8).
