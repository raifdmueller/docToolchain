...
* **Status**: Accepted (for v4) — **decided; carrier not yet implemented**. No `DtcException` class exists in the codebase today; v4 scripts still use `System.exit` + `println` (including the library `DtcConfig.load()`). The hierarchy and top-level handler below are the committed design, pending implementation (risk **R-004**). This same carrier is the missing piece for the T-002 secret-redaction mitigation (Chapter 8) and the systemic `System.exit` finding (#1627).
...
* **Decision**: Every user-recoverable error must include an **actionable remediation step** — not what went wrong, but what the user should do to fix it. To be implemented as a lightweight exception hierarchy with mandatory guidance messages:
+
[source,groovy]
----
