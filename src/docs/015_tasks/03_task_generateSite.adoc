...
Let's explore some practical examples of how to use the `generateSite` task effectively:
...
1. Parse all your AsciiDoc files in the input path (by default `src/docs`)
2. Generate a microsite with navigation structure based on your folder hierarchy
3. Output the result to `build/microsite/output/`

==== Custom Theme Usage

To use a custom theme, define it in your `dtcw` script:

[source,bash]
----
# In your dtcw or dtcw.ps1 file
export DTC_SITETHEME=https://example.com/your-theme.zip
----

Then run `generateSite` normally:
...
For CI/CD environments, add these lines to your build script:

[source,bash]
----
# Set headless mode to avoid interactive prompts
export DTC_HEADLESS=true

# Generate the site
./dtcw generateSite

# Deploy the files from build/microsite/output/ to your server
...
For large documentation projects, you might encounter performance issues. Here are some tips:

* Consider splitting your documentation into multiple repositories if it's very large
* Use the `autobuildSite` script for development but disable it in CI/CD environments
* Include only necessary images in your `imageDirs` configuration
* Use a custom `outputPath` to keep build artifacts outside your repository:
...
* *Missing theme error*: Ensure your `DTC_SITETHEME` URL is accessible. Try downloading it manually first.
* *Navigation structure issues*: Check that your folder structure follows the conventions described in the "Navigation elements" section.
* *Blank pages*: Verify that your AsciiDoc files have `:jbake-status: published` in their headers.
* *Images not showing*: Make sure your images are in folders listed in `imageDirs` and the paths in your documents are correct.
* *Custom theme not applied*: Check if the theme was downloaded correctly in `$HOME/.doctoolchain/themes/`.

If you encounter persistent issues, try running with a clean build:
